...
exports = module.exports = function (req, res) {
	...
	asyncList.push(function(callback){
		keystone.list('UserManual').model.find({'status':'published'}).sort({'sort':1}).limit(1000).exec(function (err, results) {
	...
	async.parallel(asyncList, function(err){
		...
		locals.section = 'UserManual';

		locals.title = 'UserManual|BelleMa'
		// Render the view
		view.render('user_manual');
	})
