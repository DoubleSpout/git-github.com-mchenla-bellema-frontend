...
exports = module.exports = function (req, res) {
	...
	//get side
	asyncList.push(function(callback){
		keystone.list('TroubleShooting').model.find({ 'state':'published', 'putOnSide':'yes'}).sort({'sort':1}).limit(3).exec(function (err, results) {
	...
	//recent post
	asyncList.push(function(callback){
		keystone.list('TroubleShooting').model.find({'state':'published'}).sort({'publishedDate':-1}).limit(3).exec(function (err, results) {
	...
	asyncList.push(function(callback){
		...
		keystone.list('TroubleShooting').model.find({}).count(q).exec(function (err, results) {
	...
	asyncList.push(function(callback){
		...
		keystone.list('TroubleShooting').model.find(q).sort({'sort':1}).limit(perPage).skip(skipNum).exec(function (err, results) {
	...
	async.parallel(asyncList, function(err){
		...
		locals.NavLink = '/customer/troubleshooting/info'
